#! /usr/bin/python3.10
"""
Kick VOD checker task

Checks for VODs/archives on Kick and updates announcements.
...
async def start_task(bot):
    """Entry point for Kick VOD checker task."""
    await kick_vod_checker_task(bot)


async def kick_vod_checker_task(bot):
    """Check for Kick VODs and update announcements."""
    await bot.wait_until_ready()

    logger.info("Kick VOD checker task started (5 minute interval)")

    while not bot.is_closed():
        logger.debug('Running kick_vod_checker_task')

        try:
            await _check_kick_vods(bot)
        except Exception as e:
            logger.error(f'Kick VOD checker error: {e}', exc_info=True)

        await asyncio.sleep(300)  # 5 minutes


async def _check_kick_vods(bot):
    """Check Kick announcements for available VODs."""
    ...
    try:
        ...
        if not announcements:
            logger.debug('No Kick announcements need VOD checking')
            return

        logger.info(f'Checking {len(announcements)} Kick announcements for VODs')

        # Simple session - Kick's official public API handles auth via OAuth token
        async with aiohttp.ClientSession() as session:
            for ann in announcements:
                await _check_single_kick_vod(bot, ann, kick_service, session, guild_dao, dao)
...
async def _check_single_kick_vod(bot, ann, kick_service, session, guild_dao, dao):
    """Check for Kick VOD for a specific announcement."""
    try:
        ...
        vod_settings = settings.get('kick', {}).get('vod_settings', {})
        if not vod_settings:
        ...
        # Check for VOD
        vod_url = await kick_service.find_vod_for_stream(
            session,
            streamer_username,
            stream_started_at
        )
        ...
        if vod_url:
            logger.info(f"Found Kick VOD for {streamer_username} in guild {guild_id}: {vod_url}")
            dao.update_vod_info(ann_id, vod_url)

            if vod_settings.get('edit_message_when_vod_available', True):
                await _edit_kick_announcement_with_vod(bot, guild_id, channel_id, message_id, vod_url, ann)
        else:
            ...
            if attempt_count % 5 == 0 and attempt_count > 0:
                logger.warning(
                    f"Still checking for Kick VOD: {streamer_username} in guild {guild_id} "
                    f"(attempt {attempt_count + 1})"
                )

    except Exception as e:
        logger.error(f"Error checking Kick VOD for announcement: {e}", exc_info=True)


async def _edit_kick_announcement_with_vod(bot, guild_id, channel_id, message_id, vod_url, ann):
    """Edit the Kick announcement message to include VOD link."""
    try:
        ...
        logger.info(f"Updated Kick announcement {message_id} with VOD link")

    except Exception as e:
        logger.error(f"Error editing Kick announcement with VOD: {e}", exc_info=True)
