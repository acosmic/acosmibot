...
class Slots(commands.Cog):
    ...
    async def slots(self, interaction: discord.Interaction, bet: typing.Literal[100, 1000, 5000, 10000] = 100):
        ...
        if slot1 == slot2 == slot3 == "<:acosmicD:1171219346299814009>":
            ...
            embed.description = f"# {result}\nSUPER JACKPOT! You won {amount_won:,.0f} credits and 1,000 EXP! <a:peepoRich:855180219772960798>"
            embed.color = discord.Color.gold()
            general_embed = discord.Embed()
            general_embed.title = f"🎰 {interaction.user.name} hit the SUPER Jackpot! 🎰"
            general_embed.description = f"# {result}\n{interaction.user.mention} hit the SUPER Jackpot and won {amount_won:,.0f} credits! <a:peepoRich:855180219772960798>"
        ...
        if slot1 == slot2 == slot3 == "🦄":
            ...
            embed.description = f"# {result}\nMEGA JACKPOT! You won {amount_won:,.0f} credits and 500 EXP! <a:peepoRich:855180219772960798>"
            embed.color = discord.Color.gold()
            general_embed = discord.Embed()
            general_embed.title = f"🎰 {interaction.user.name} hit the MEGA Jackpot! 🎰"
            general_embed.description = f"# {result}\n{interaction.user.mention} hit the MEGA Jackpot and won {amount_won:,.0f} credits! <a:peepoRich:855180219772960798>"
            general_embed.color = discord.Color.gold()
            general_embed.set_footer(text="Try your luck with /slots! in 🎰︱casino")
            await general_channel.send(embed=general_embed)
            ...
            vgain = cost * 0.01 # 1% of the lost credits
            vcredits += vgain
            ...
            amount_lost = cost
            embed.title = f"😢 {interaction.user.name}'s Slot Machine Result 😢"
            embed.description = f"# {result}\nNo match, better luck next time! You lost {amount_lost:,.0f} credits, but gained 2 EXP.\n\n{vgain:,.0f} Credits have been added to the vault! 🏦"
