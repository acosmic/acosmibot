...
class Slots(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.jail_exp_penalty_rate = 0.10  # 10% of user's global XP
    ...
    async def slots(self, interaction: discord.Interaction, bet: typing.Literal[100, 1000, 5000, 10000, 25000] = 100):
        ...
        if current_user is None:
            await interaction.response.send_message(
                "You need to send a message first to initialize your global account!", ephemeral=True)
            return
        ...
        if current_guild_user.currency < cost and not is_bonus_spin:
        ...
        # JAIL POT
            ...
            exp_lost = math.ceil(current_user.global_exp * self.jail_exp_penalty_rate)
            amount_lost = math.ceil(current_guild_user.currency * self.jail_currency_penalty_rate)
        ...
        new_event = SlotEvent(0, interaction.user.id, slot1, slot2, slot3, bet_amount, amount_won, amount_lost,
                              timestamp)

        # Update guild user
        current_guild_user.currency += amount_won
        current_guild_user.currency -= amount_lost
        current_guild_user.exp += exp_gained
        current_guild_user.exp_gained += exp_gained
        current_guild_user.exp_lost += exp_lost

        # Update global user
        current_user.global_exp += exp_gained
        current_user.global_exp -= exp_lost
        if current_user.global_exp < 0:
            current_user.global_exp = 0  # Prevent negative global exp
